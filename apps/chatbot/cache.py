...
def record_usage(provider, amount):
    ...
    try:
        ...
        pipe.expire(key, window, nx=True)  # only set TTL if not already set, so the window doesn't keep sliding forward on every call
        pipe.execute()
    except redis.RedisError as e:
...
def mark_exhausted(provider):
    ...
    try:
        client = _get_client()
        pipe = client.pipeline()
        pipe.set(f'usage:{provider}', budget)
        pipe.expire(f'usage:{provider}', window, nx=True)
        pipe.execute()
    except redis.RedisError as e:
